...
import { createFiroRpcClient, RpcCallError } from '../lib/index.js';
const { parsed } = dotenv.config({
  path: '.env.test',
});
const { RPC_HOST, RPC_USER, RPC_PASS, RPC_PORT, RPC_PROTOCOL } = parsed as Record<string, string>;
...
describe('FiroRpcClient (integration)', () => {
  const client = createFiroRpcClient({
    ...
    user: RPC_USER!,
    pass: RPC_PASS!,
    protocol: RPC_PROTOCOL as 'http' | 'https',
  });
